...
export class FaqAiService {
  ...
  static async chooseSupportFaqCandidate(params: {
    ...
    const prompt = [
      ...
      'Choose should_auto_reply=true only if exactly one candidate FAQ fully and directly answers the user message, OR if the matching FAQ has an AI agent (agent_enabled: true) that covers the intent of the message (e.g. checking stock for specific models).',
      'IMPORTANT: If a candidate FAQ has agent_enabled: true, it means an AI agent with live tools will take over. Therefore, you should enthusiastically route specific questions (like stock checks for specific items) to the agent FAQ even if the FAQ answer text is generic.',
      ...
      'If the user message needs a human, account-specific help, sales follow-up, troubleshooting, order review, or none of the FAQs clearly fit (and there is no suitable agent FAQ), return should_auto_reply=false.',
