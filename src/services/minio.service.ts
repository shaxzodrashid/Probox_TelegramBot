...
class MinioServiceClass {
  ...
  async getFileAsBuffer(objectName: string): Promise<Buffer> {
    try {
      logger.info(
        `[MINIO] getFileAsBuffer: getting object ${objectName} from bucket ${this.bucket}`,
      );
  ...
  async getFileAsStream(objectName: string): Promise<Readable> {
    try {
      logger.info(
        `[MINIO] getFileAsStream: getting object ${objectName} from bucket ${this.bucket}`,
      );
      const dataStream = await this.client.getObject(this.bucket, objectName);
  ...
  async uploadFile(
    objectName: string,
    streamOrBuffer: Buffer | Readable,
    ...
    try {
      ...
      if (Buffer.isBuffer(streamOrBuffer)) {
        ...
        result = await (this.client.putObject as any)(
          this.bucket,
          objectName,
          streamOrBuffer,
          undefined,
  ...
  async uploadUserPassport(
    ...
    try {
      ...
      const namePart = userDetails
        ? `_${userDetails.first_name || ''}_${userDetails.last_name || ''}`.replace(
            /[^a-zA-Z0-9]/g,
            '_',
          )
        : '';
      ...
      await Promise.all(
        bufferArray.map(async (buffer, i) => {
          ...
          logger.info(
            `[MINIO] Compression done: ${buffer.length} -> ${compressedBuffer.length} bytes`,
          );

          // 4. Upload compressed one
          await this.uploadFile(filename, compressedBuffer);
          logger.info(
            `[MINIO] Handled passport update for user ${telegramId}: old deleted, new compressed and uploaded as ${filename}`,
          );
        }),
      );
    } catch (error) {
  ...
  async uploadFaceId(telegramId: number, buffer: Buffer): Promise<void> {
    ...
    try {
      ...
      const compressedBuffer = await sharp(buffer).jpeg({ quality: 50, mozjpeg: true }).toBuffer();
      ...
      await this.uploadFile(filename, compressedBuffer);
      logger.info(
        `[MINIO] Handled face ID upload for user ${telegramId}: old deleted, new compressed and uploaded as ${filename}`,
      );
    } catch (error) {
      logger.error(
