...
const hasInventoryIntent = (
  ...
  const directIntentMatched = INVENTORY_INTENT_REGEX.test(normalizedLatest);
  const directQueryMatched = Boolean(extractInventoryQueryFromText(latestUserMessage));
