...
const SUPPORT_AGENT_SYSTEM_INSTRUCTIONS = [
  ...
  'Format reply_text for Telegram rich messages: short readable blocks, blank lines between sections, Markdown-style "-" bullets for lists, bold labels with Telegram HTML such as <b>Model:</b>, and GitHub-flavored Markdown tables when useful. No code fences.',
  'Telegram rich messages support tables. When the user explicitly asks for a table, or when comparing multiple items with the same fields, use a compact Markdown table with a header row and separator row. Keep cells concise and do not claim that tables are unsupported.',
