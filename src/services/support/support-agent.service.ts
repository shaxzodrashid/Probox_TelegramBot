...
const SUPPORT_AGENT_SYSTEM_INSTRUCTIONS = [
  ...
  'Format reply_text for Telegram rich messages: short readable blocks, blank lines between sections, Markdown-style "-" bullets for lists, bold labels with Telegram HTML such as <b>Model:</b>, and GitHub-flavored Markdown tables as the default format for multiple product results. No code fences.',
  'Telegram rich messages support tables. Use one compact Markdown table with a header row and separator row whenever the answer contains multiple product models or multiple variants of the same model. This includes an exact model request such as "iPhone 17 Pro" when grounded results contain different colors, memory sizes, SIM types, conditions, stores, or prices.',
  'Table columns are completely dynamic: include the useful grounded fields for the current results, and put every distinct data attribute in its own column. Do not combine values such as color, memory, condition, price, store, or stock count into one cell when they can be separate columns. Each row must represent one concrete model or purchasable variant.',
  'When the customer asks about several different models in one request, or asks about different models consecutively and the grounded context contains their details, consolidate the comparison into a single table instead of separate tables or repeated prose.',
  'Example table shape:\nModel | Holati | Rangi | Xotirasi | Narxi (UZS)\n--- | --- | --- | --- | ---\niPhone 17 Pro | Yangi | Cosmic Orange | 256GB | 17 446 000\niPhone 16 Pro | Yangi | Natural Titanium | 256GB | 20 519 000\niPhone 15 Pro | B/U | Blue Titanium | 256GB | 8 086 900',
  'Keep table cells concise. Use prose or bullets for a single result unless the customer explicitly asks for a table, and do not claim that tables are unsupported.',
  'For product availability, price, installment, or branch answers with only one result, prefer this shape when grounded: one direct summary sentence, then bullets grouped by branch/option/payment detail, then one concise CTA. For multiple results, use the table rules above.',
  ...
  '</output>',
  '<contacts>',
  'When giving Call-Center contacts, provide these phone numbers exactly: +998781134774 and +998555134774.',
  '</contacts>',
  '<product_clarification>',
  'Before giving SAP availability, price, branch, or stock details for Apple products, make sure the requested product identifies at least the product family, series/generation, and exact variant/type.',
  ...
  'After the exact model/series/type is clear, if grounded inventory already contains multiple storage, color, SIM type, condition, store, or price variants, show those variants in one Markdown table instead of asking the customer to choose storage or color first. If no grounded variant list is available and storage or color is required for a safe lookup, ask a short clarification.',
  ...
  'Do not use inventory pre-check results or call lookup_store_items to answer a request that is still ambiguous at the product family, series/generation, or exact variant/type level. Missing storage or color alone is not ambiguous when the exact model/type is clear and grounded results can present the available variants in a table.',
  '</product_clarification>',
  '<tool_policy>',
  ...
  'If inventory pre-check already answers a specific model/type request or provides its available variants, use it. Otherwise use lookup_store_items for specific clarified products and lookup_available_devices for broad catalog questions.',
...
const lookupStoreItemsTool: GeminiTool = {
  declaration: {
    ...
    description:
      'Looks up live item availability in Probox stores using SAP search behavior and exact structured filters. Use this for questions about whether a clarified item is in stock, which store has it, stock counts, warehouse checks, item-code lookups, or IMEI searches. Before product lookups, clarify product family, series/generation, and exact variant/type. Storage and color may remain unspecified when the customer wants to see available variants for an exact model/type. For product lookups, pass structured model/device_type/memory/color/sim_type/condition when known; use search for item-code, IMEI, or extra free-text only. Do not pass slang or full customer sentences. Examples: model="iPhone 17", device_type="Pro Max", memory="256GB", color="Deep Blue", sim_type="nano-SIM", condition="Yangi"; or model="iPhone 17", device_type="Pro"; or search="123456".',
    strict: true,
    parameters: {
      type: 'object',
      properties: {
...
export class SupportAgentService {
  static async generateReply(params: {
    ...
    const prompt = [
      ...
      'When an exact model/type has multiple grounded variants, present them in one Markdown table with dynamic columns and one data attribute per column; do not ask for memory or color first.',
      'When several different requested models have grounded results, consolidate them into one comparison table.',
