...
export class SupportService {
  ...
  private static normalizeTicket(ticket: SupportTicket | null | undefined): SupportTicket | null {
    ...
    return {
      ...
      message_id:
        ticket.message_id === undefined || ticket.message_id === null
          ? undefined
          : this.toSafeNumber(ticket.message_id),
      group_message_id:
        ticket.group_message_id === undefined || ticket.group_message_id === null
          ? undefined
          : this.toSafeNumber(ticket.group_message_id),
      handling_mode: (ticket.handling_mode || 'human') as SupportHandlingMode,
      matched_faq_id:
        ticket.matched_faq_id === undefined || ticket.matched_faq_id === null
          ? null
      ...
      replied_by_admin_id:
        ticket.replied_by_admin_id === undefined || ticket.replied_by_admin_id === null
          ? undefined
  ...
  private static normalizeMessage(message: SupportTicketMessage): SupportTicketMessage {
    return {
      ...
      telegram_message_id:
        message.telegram_message_id === undefined || message.telegram_message_id === null
          ? null
          : this.toSafeNumber(message.telegram_message_id),
      group_message_id:
        message.group_message_id === undefined || message.group_message_id === null
          ? null
  ...
  static async createTicket(params: CreateTicketParams): Promise<SupportTicket> {
    ...
    while (attempts < maxAttempts) {
      const existing = await db('support_tickets').where('ticket_number', ticketNumber).first();
   ...
   */
  static async updateGroupMessageId(ticketId: number, groupMessageId: number): Promise<void> {
   ...
   */
  static async getTicketByGroupMessageId(groupMessageId: number): Promise<SupportTicket | null> {
    // Try cache first
    const cacheKey = `${this.TICKET_CACHE_PREFIX}${groupMessageId}`;
    const cachedTicketId = await redisService.get<string>(cacheKey);

    if (cachedTicketId) {
      const ticket = await db('support_tickets').where('id', parseInt(cachedTicketId, 10)).first();
      return this.normalizeTicket(ticket);
    }

    // Fallback to database
    const ticket = await db('support_tickets').where('group_message_id', groupMessageId).first();
   ...
   */
  static async getTicketByTicketNumber(ticketNumber: string): Promise<SupportTicket | null> {
  ...
  static async getOpenAgentTicketByUserTelegramId(
  ...
  static async appendMessage(params: {
    ...
    await db('support_tickets').where('id', params.ticketId).update({
      updated_at: new Date(),
    });
  ...
  static async escalateAgentTicket(
    ticketId: number,
    reason: string,
  ): Promise<SupportTicket | null> {
    const [result] = await db<SupportTicket>('support_tickets')
   ...
   */
  static async getUserOpenTickets(userTelegramId: number): Promise<SupportTicket[]> {
  ...
  static async getAllTickets(): Promise<SupportTicketWithUser[]> {
    const tickets = await db('support_tickets')
      ...
      .select('support_tickets.*', 'users.first_name', 'users.last_name', 'users.phone_number')
