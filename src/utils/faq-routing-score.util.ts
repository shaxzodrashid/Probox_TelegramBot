...
const getConcepts = (tokens: string[]): Set<string> => {
  ...
  for (const [concept, members] of Object.entries(CONCEPT_GROUPS)) {
    if (members.some((member) => tokenSet.has(canonicalizeToken(member)))) {
...
const computeIntentAdjustments = (
  ...
  if (queryConcepts.has('branch') && queryConcepts.has('current_availability') && candidateConcepts.has('count')) {
    adjustment -= 0.12;
  }
