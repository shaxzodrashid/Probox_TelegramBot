...
export const normalizeInventoryText = (value: string): string =>
  value
    ...
    .replace(/(?:\b|(?<=^|\s))(?:ayfon|aifon|айфон)(?:\b|(?=$|\s))/g, 'iphone')
    .replace(/(?:\b|(?<=^|\s))(?:aypad|aipad|айпад)(?:\b|(?=$|\s))/g, 'ipad')
    .replace(/(?:\b|(?<=^|\s))(?:makbuk|macbuk|макбук)(?:\b|(?=$|\s))/g, 'macbook')
    .replace(/(?:\b|(?<=^|\s))(?:erpods|earpods|эйрподс|аирподс)(?:\b|(?=$|\s))/g, 'airpods')
    .replace(/(?:\b|(?<=^|\s))(?:apple\s*watch|apel\s*watch|эпл\s*вотч)(?:\b|(?=$|\s))/g, 'apple watch')
