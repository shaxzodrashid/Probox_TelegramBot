...
async function main() {
  ...
  try {
    ...
    logger.info(`--- 📦 [1/2] Fetching Installments for DocEntry: ${docEntry} ---`);
    const testInstSql = `
    ...
    logger.info(`--- 💳 [2/2] Fetching Fully Paid Dates for DocEntry: ${docEntry} ---`);
    ...
    if (payments.length === 0) {
      ...
      logger.info('--- 🎯 Comparison (On-Time Analysis) ---');
